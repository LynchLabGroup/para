...
expression.modifier <- function(extract_file, exp_file) {
  ...
  list(mod=classifier, match=length(extract_match[,1]), nomatch=length(extract_no[,1]))
}
```
...
for (i in 1:length(extract_bi)) {
  ...
  bi.classify = rbind(bi.classify ,cbind(motif_data, as.factor(modifier$mod), modifier$match, modifier$nomatch))
