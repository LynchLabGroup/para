...
rule gene_tree:
    ...
    input: "bin/scripts/editnewick.py", "{familyCDS}.phyl_phyml_tree.txt"
    output: "{familyCDS}.newick"
    shell: "python2 {input} {output}"

rule phyml_tree:
    """Compute ML gene tree."""
    input: "{familyCDS}.phyl"
    output: "{familyCDS}.phyl_phyml_tree"
    shell: "phyml -i {input}"

rule fasta_to_phylip:
    """Convert Fasta to Phylip sequences."""
    input: "{familyCDS}.e.fasta"
    output: "{familyCDS}.phyl"
    shell: "perl bin/scripts/ConvertFastatoPhylip.pl {input} {output}"

rule match_seqs:
    """Matching sequences between CDS and family."""
    input: "bin/scripts/extractmatch.py", "{family}.fasta", "{familyCDS}.fasta"
    output: "{familyCDS}.e.fasta"
    shell: "python2 {input} {output}"

rule transform_CDS_header:
    """Reduce CDS header."""
    input: "bin/scripts/fastaheader.py", "{familyCDS}.nt_ali.fasta"
    output: "{familyCDS}.fasta"
    shell: "python2 {input} '|' {output}"
